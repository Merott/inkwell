...
function buildMetadata(
  ...
  const publishedAt =
    jsonLd?.datePublished ?? ogTags["article:published_time"] ?? new Date().toISOString();
...
function convertElement(
  ...
  if (tag === "h2" || tag === "h3" || tag === "h4")
    return convertHeading($el, parseInt(tag[1]!));
...
function convertBookmark(
  ...
  const title =
    $el.find(".kg-bookmark-title").text().trim() || "Bookmark";
  return {
    type: "paragraph",
