...
describe('matches', () => {
  it('matches 404media.co URLs', () => {
    expect(ghostSource.matches('https://www.404media.co/some-article/')).toBe(
      true,
    )
    expect(ghostSource.matches('https://404media.co/some-article/')).toBe(true)
    expect(ghostSource.matches('http://www.404media.co/another-post/')).toBe(
      true,
    )
  })
...
describe('parse', () => {
  ...
  it('contains paragraphs with html format', () => {
    ...
    expect((paragraphs[0] as Paragraph).format).toBe('html')
  })
  ...
  it('contains headings', () => {
    ...
    const h2 = headings.find((h) => (h as Heading).level === 2)
    expect(h2).toBeDefined()
    expect((h2 as Heading).text).toContain('Reader-Supported')
  })

  it('contains h3 headings', () => {
    const h3s = article.body.filter(
      (c) => c.type === 'heading' && (c as Heading).level === 3,
    )
  ...
  it('contains unordered list', () => {
    const lists = article.body.filter(
      (c) => c.type === 'list' && (c as List).style === 'unordered',
    )
    expect(lists.length).toBeGreaterThan(0)
    expect((lists[0] as List).items.length).toBeGreaterThanOrEqual(3)
  })

  it('contains ordered list', () => {
    const lists = article.body.filter(
      (c) => c.type === 'list' && (c as List).style === 'ordered',
    )
    expect(lists.length).toBeGreaterThan(0)
  })
  ...
  it('contains blockquote', () => {
    ...
    expect((quotes[0] as Blockquote).text).toContain('accountable')
  })
  ...
  it('contains embed from kg-embed-card', () => {
    ...
    const embed = embeds[0] as EmbedComponent
  ...
  it('contains video from kg-video-card', () => {
    ...
    const video = videos[0] as VideoComponent
  ...
  it('contains bookmark as paragraph link', () => {
    ...
    const bookmark = paragraphs.find((p) =>
      (p as Paragraph).text.includes('Ghost for Independent Journalism'),
    )
    expect(bookmark).toBeDefined()
    expect((bookmark as Paragraph).text).toContain('<a href=')
  })

  it('contains codeBlock from pre>code', () => {
    const codeBlocks = article.body.filter((c) => c.type === 'codeBlock')
    expect(codeBlocks.length).toBeGreaterThan(0)
    const block = codeBlocks[0] as CodeBlock
  ...
  it('converts callout card to blockquote', () => {
    const quotes = article.body.filter((c) => c.type === 'blockquote')
    const callout = quotes.find((q) =>
      (q as Blockquote).text.includes('Subscribe'),
    )
