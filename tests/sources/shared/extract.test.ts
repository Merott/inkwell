...
import {
  ...
  extractJsonLd,
  extractOgTags,
} from '@/sources/shared/extract.ts'
...
describe('detectEmbedPlatform', () => {
  ...
  it('detects Instagram', () => {
    expect(detectEmbedPlatform('https://www.instagram.com/p/abc/embed')).toBe(
      'instagram',
    )
  })

  it('detects TikTok', () => {
    expect(detectEmbedPlatform('https://www.tiktok.com/embed/v2/abc')).toBe(
      'tiktok',
    )
  })
  ...
  it('detects X/Twitter', () => {
    ...
    expect(detectEmbedPlatform('https://platform.x.com/embed/Tweet.html')).toBe(
      'x',
    )
  })
